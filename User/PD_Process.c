...
void PDO_Request (UINT8 pdo_index) {
    ...
    if ((pdo_index > PDO_Len) || (pdo_index == 0)) {
        ...
        drv_prv.pd_status = 1;
        drv_prv.pd_voltage = Voltage;
        drv_prv.pd_current = Current;
        printf ("Request:\r\nCurrent:%d mA\r\nVoltage:%d mV\r\n", Current, Voltage);
...
int compfunc (const void *a, const void *b) {
    return ((PDO_Analyse_Result *)b)->Voltage - ((PDO_Analyse_Result *)a)->Voltage;
}

void PD_SELECT_PDO (UINT8 *srccap) {
    PDO_Analyse_Result results[PDO_Len];

    /* Analysis of the voltage and current of each PDO group */
    for (int var = 1; var <= PDO_Len; ++var) {
        results[var - 1].INDEX = var;
        PD_PDO_Analyse (var, srccap, &results[var - 1].Current, &results[var - 1].Voltage);
        printf ("PDO:%d\r\nCurrent:%d mA\r\nVoltage:%d mV\r\n", var, results[var - 1].Current, results[var - 1].Voltage);
    }

    qsort (results, PDO_Len, sizeof (PDO_Analyse_Result), compfunc);
    UINT8 Select_Index = 1;
    for (int var = 0; var < PDO_Len; ++var) {
        if ((results[var].Current / 1000.0) * (results[var].Voltage / 1000.0) >= PD_MIN_POWER) {
            Select_Index = results[var].INDEX;
            break;
        }
    }
